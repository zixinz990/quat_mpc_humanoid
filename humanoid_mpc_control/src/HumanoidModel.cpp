...
namespace mpc {
void HumanoidModel::ct_srb_quat_dyn(double *x_dot, const double *x, const double *u, Eigen::Matrix<double, 3, 2> foot_pos_torso, Eigen::Matrix3d robot_inertia, double robot_mass, Eigen::Matrix3d torso_rot_mat) const {
    ...
    g_vec << 0, 0, -9.81;
    g_vec_body = torso_rot_mat.transpose() * g_vec;
    moment_body = Utils::skew(foot_pos_torso.block<3, 1>(0, 0)) * u_vec.segment<3>(0) +
                  Utils::skew(foot_pos_torso.block<3, 1>(0, 1)) * u_vec.segment<3>(3);
    ...
    x_dot_vec.segment<3>(7) = (u_vec.segment<3>(0) + u_vec.segment<3>(3)) / robot_mass + g_vec_body;
...
void HumanoidModel::ct_srb_quat_jac(double *jac, const double *x, const double *u, Eigen::Matrix<double, 3, 2> foot_pos_torso, Eigen::Matrix3d robot_inertia, double robot_mass) const {
    (void)u;
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        dfc_du.block<3, 3>(10, 3 * i) = robot_inertia.inverse() * Utils::skew(foot_pos_torso.block<3, 1>(0, i));
    }
